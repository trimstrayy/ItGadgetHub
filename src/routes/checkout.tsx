        ...
        <div className="lg:col-span-2">
          ...
          <div className="glass-strong rounded-2xl p-6 mt-4">
            <AnimatePresence mode="wait">
              {step === 0 && (
                <motion.div key="ship" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    ...
                    <Field label="City" defaultValue="Kathmandu" />
              ...
              {step === 2 && (
                <motion.div key="rev" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-3">
                  ...
                  {items.map((i) => (
                    <div key={i.product.id} className="flex gap-3 glass rounded-xl p-3">
            ...
            <div className="mt-6 flex justify-between gap-3">
              ...
              {step < STEPS.length - 1 ? (
                <button onClick={() => setStep(step + 1)} className="inline-flex items-center gap-2 px-5 py-2 rounded-lg bg-primary text-primary-foreground font-semibold">
                  Continue <ArrowRight className="h-4 w-4" />
                </button>
              ) : (
                <button onClick={() => { setDone(true); clear(); toast.success("Order placed!"); }}
...
function Field({ label, full, type = "text", defaultValue }: { label: string; full?: boolean; type?: string; defaultValue?: string }) {
  return (
    <label className={full ? "sm:col-span-2 block" : "block"}>
      <span className="text-[10px] uppercase tracking-widest text-muted-foreground">{label}</span>
      <input type={type} defaultValue={defaultValue}
        className="mt-1 w-full bg-transparent border border-white/10 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-primary" />
    </label>
  );
}
